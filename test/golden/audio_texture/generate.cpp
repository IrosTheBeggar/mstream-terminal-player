// Golden vectors for src/shader/audio.rs, from the reference itself.
//
// Links mstream_music's android/app/src/main/cpp/audio_texture.cpp —
// unmodified — against the stub GLES in stubs/, feeds it a signal, and
// writes each texture it uploads. See README.md for how to run it.
//
// The signal is built from integer arithmetic and float operations whose
// results are exact or singly rounded, so the Rust test can generate the
// same samples bit for bit rather than reading 47 KB of them from disk; the
// FNV-1a hash in the header is how it proves it did.
