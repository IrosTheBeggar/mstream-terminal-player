//! The admin panel: `mstream-player admin`. One room for now — Libraries,
//! the server's music folders — drawn full-screen from the UI kit the way
...
//! pattern, ops carrying their own data).
...
impl Room {
    fn new(client: Client, same_machine: bool) -> Self {
        Room {
    ...
    /// The server's host, for the header — never the whole URL.
    fn host(&self) -> String {
    ...
    fn dispatch_queued(&mut self, to_worker: &Sender<(Arc<Client>, Op)>) {
        if self.in_flight {
        ...
        self.in_flight = true;
        if to_worker.send((self.client.clone(), op)).is_err() {
    ...
    fn apply(&mut self, done: Done) {
        ...
        match done {
            ...
            Done::Loaded(Err(e)) => self.note = Some((gate_message(&e), true)),
...
/// What a failed listing means, said plainly: the two gates B4 warns a
/// terminal client will hit (a non-admin account or an address restriction
/// → 403, `lockAdmin` → 405), and the plain sign-in miss.
fn gate_message(e: &ApiError) -> String {
    match e {
        ...
        other => format!("{}: {other}", t!("admin.load_failed")),
    }
}
...
pub fn run(args: AdminArgs) -> i32 {
    ...
    let mut room = Room::new(client, args.same_machine);
    crate::setup::boot_language();
    room.queue(Op::Load, t!("admin.busy_loading"));
    run_tui(room)
}

/// The terminal session around the room: ground lease, mouse capture,
/// pointer contract, event loop, teardown — the wizard's, minus pictures.
fn run_tui(mut room: Room) -> i32 {
    ...
    let outcome = event_loop(&mut terminal, &mut room, mouse_on, &to_worker, &from_worker);
    if mouse_on {
...
fn event_loop(
    ...
    let mut hand = false;
    loop {
        terminal.draw(|frame| render(frame, room))?;

        // Fold in whatever the worker finished, then hand it the next op.
        loop {
            match from_worker.try_recv() {
                Ok(done) => room.apply(done),
                Err(TryRecvError::Empty) => break,
                Err(TryRecvError::Disconnected) => {
                    room.note = Some((t!("note.worker_gone").to_string(), true));
                    break;
                }
            }
        }
        room.dispatch_queued(to_worker);

        let over = room.ui.hovering_clickable();
        if over != hand {
            hand = over;
            set_pointer_shape(hand, mouse_on);
        }
        if let Some(act) = room.ui.hold_action() {
            room.act(act);
        }
        room.ui.dwell_tick();
        ...
        for input in inputs {
            match input {
                TermEvent::Key(key) if key.kind == KeyEventKind::Press => {
                    room.ui.dismiss_tooltip();
                    if key.modifiers.contains(KeyModifiers::CONTROL)
                        && key.code == KeyCode::Char('c')
                    {
                        return Ok(Outcome::Quit);
                    }
                    if let Some(outcome) = handle_key(room, key) {
                        return Ok(outcome);
                    }
                }
                TermEvent::Mouse(mouse) => {
                    ...
                    match mouse.kind {
                        MouseEventKind::Down(MouseButton::Left) => {
                            if !room.ui.begin_press(at) {
                                continue;
                            }
                            if let Some(act) = room.ui.hit(at)
                                && let Some(outcome) = room.act(act)
                            {
                                return Ok(outcome);
                            }
                            room.ui.arm_bars(at);
                        }
                        MouseEventKind::Moved => room.ui.motion(at),
                        MouseEventKind::Drag(_) => {
                            room.ui.motion(at);
                            if let Some(act) = room.ui.drag_action(at) {
                                room.act(act);
                            }
                        }
                        MouseEventKind::Up(_) => room.ui.release(),
                        MouseEventKind::ScrollUp | MouseEventKind::ScrollDown => {
                            room.ui.pointer = Some(at);
                            ...
                            match &mut room.modal {
                                ...
                                Modal::None => {
                                    room.tscroll = if up {
                                        room.tscroll.saturating_sub(1)
                                    } else {
                                        room.tscroll.saturating_add(1)
                                    };
                                }
                                _ => {}
                            }
...
fn render(frame: &mut Frame, room: &mut Room) {
    room.ui.begin_frame();
    let area = frame.area();
    // The fixed scheme paints its own ground — only when the terminal
    // granted OSC 11 ownership of the whole window (see the wizard).
    ...
    // The heading, and the server this room manages.
    let head = Rect { x: 2, y: 0, width: area.width.saturating_sub(4), height: 1 };
    frame.render_widget(Paragraph::new(Span::styled(t!("admin.title").to_string(), bold())), head);
    frame.render_widget(
        Paragraph::new(Span::styled(format!("{} · {}", room.host(), t!("admin.role")), dim()))
    ...
    // Status note (errors, busy) above the keyboard tips, on the bottom edge.
    let note_y = area.height.saturating_sub(2);
    let tips_y = area.height.saturating_sub(1);
    let line = Rect { x: 2, y: note_y, width: area.width.saturating_sub(4), height: 1 };
    if let Some((text, is_err)) = room.note.clone() {
        let style = if is_err { Style::default().fg(th().gold) } else { dim() };
        frame.render_widget(Paragraph::new(Span::styled(text, style)), line);
    }
    if let Some(busy) = room.busy.clone() {
        frame.render_widget(Paragraph::new(Span::styled(busy, accent())), line);
    }
    let tips = Rect { x: 2, y: tips_y, width: area.width.saturating_sub(4), height: 1 };
    frame.render_widget(Paragraph::new(Span::styled(footer_hint(room), dim())), tips);
